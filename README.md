...
О том, что такое ошибки (исключения) и зачем они нужны в Javascript, можно почитать в [соответствующей статье](https://learn.javascript.ru/custom-errors). В дополнение к информации из учебника, можно добавить, что условно ошибки можно разделить на три типа: ожидаемые, неожиданные и контрольные.
...
_Контрольные ошибки_ - особый класс ошибок, которые разработчик вызывает самостоятельно, чтобы убедиться, что его код используется корректно. Проще всего понять эту концепцию на примере. Допустим, у нас есть функция, которая возвращает текстовое описание значения из справочника - `getStatusText(status: Status)`. Допустим, мы имеем следующий код:
...
// Загружает статус с сервера и возвращает его описание.
...
Здесь мы предполагаем, что сервер может вернуть только три значения: `0`, `1`, и `2`. Но что будет, если бэкенд изменит спецификацию или сломается? Функция `loadStatusText` будет возвращать `"Done"` так, как если бы приложение работало корректно. Это, в свою очередь, приведёт к багам в других частях кода, причём определить, чем они были вызваны, будет непросто. Исправить ситуацию можно, использовав контрольное исключение:
...
const getStatusText = (status: Status) => {
  switch (status) {
    ...
    default:
      throw new UnreachableError(`Unsupported status ${status}`);
  }
};
```

Настоятельно рекомендуется использовать контрольные ошибки хотя бы в методах, работающих со внешними сервисами и библиотеками, и в программных интерфейсах собственных модулей.
...
Библиотека предоставляет несколько готовых типов исключений. Следует заметить, что все ошибки из этого списка - контрольные.

#### [ExpectationError](https://github.com/devim-front/error/blob/master/docs/classes/expectationerror.md)

Эта ошибка возникает, когда переменная в коде принимает неожиданное или неподдерживаемое значение. Это исключение, к примеру, может быть использовано при проверке валидности входных параметров:
...
const parsePhone = (value: string) => {
  ...
  if (match == null) {
    // parsePhone(value) must be in format "+7 (000) 000-00-00"; got ...
    throw new ExpectationError(
      'parsePhone(value)',
      'in format "+7 (000) 000-00-00",
      value,
    );
  }
...
#### [UnreachableError](https://github.com/devim-front/error/blob/master/docs/classes/unreachableerror.md)

Указывает, что при выполнении кода движок попал на строку, попасть на которую был не должен. К примеру, в коде происходит разбор некой строки регулярным выражением. Известно, что строка гарантировано должна ему соответствовать. Но всё же следует проверить совпадение на `null` на случай, если в регулярном выражении допущена ошибка, либо передаваемая строка имеет неверный формат из-за какого-то бага:
...
```typescript
// fetchArticles.ts
import { ExpectationError } from '@devim-front/error';
import { NetworkError } from './NetworkError';
import { ParsingError } from './ParsingError';
import { ServerError } from './ServerError';
import { NotFoundError } from './NotFoundError';
...
export const fetchArticles = (category: number) => {
  if (Number.isInteger(category) || category <= 0) {
    // throws "fetchArticles(category) must be a valid ID; got <actual value>
    throw new ExpectationError(
